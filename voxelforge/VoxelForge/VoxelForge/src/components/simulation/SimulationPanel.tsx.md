...
export const SimulationPanel: React.FC = () => {
  ...
  const runPhysicsStep = (time: number) => {
    ...
    if (domain === 'mechanical') {
      // Rotate gears and shafts based on time!
      const inputRotation = time * 2; // input speed
      const outputRotation = -time * (2 / 2.2); // output speed based on reduction ratio
      ...
      if (Math.floor(time) % 15 === 0 && Math.floor(time) !== 0) {
        ...
        if (!events.some(e => e.id === eventId)) {
          addEvent({
            ...
            time: checkSec,
            type: 'info',
            description: `Vibration and gear tooth harmonics alignment verified at ${checkSec}s.`,
          });
        }
      }
    } 
    else if (domain === 'anatomy') {
      // Pulsing ventricular chamber sizes and blood velocities
      const heartRate = 75; // beats per minute
      const beatFrequency = heartRate / 60;
      const angle = 2 * Math.PI * beatFrequency * time;
      
      // Pulse scale of chambers
      ...
      setMetrics({
        'Heart Rate': '75 bpm',
        'Mitral Flow Velocity': `${(18 + Math.max(0, Math.sin(angle)) * 65).toFixed(0)} cm/s`,
        'Aortic Flow Velocity': `${(12 + Math.max(0, Math.sin(angle - Math.PI/2)) * 120).toFixed(0)} cm/s`,
        'Systolic Phase': Math.sin(angle - Math.PI/2) > 0 ? 'Systole' : 'Diastole'
      });

      // Ventricle compression trigger events
      if (Math.abs(Math.sin(angle - Math.PI/2) - 1.0) < 0.05) {
        ...
        if (!events.some(e => e.id === eventId)) {
          addEvent({
            ...
            type: 'info',
            description: `Systole Peak (Beat #${beatNum}) - Aortic valve opens, mitral valve closes.`,
          });
        }
      }
    }
    else if (domain === 'architecture') {
      // Crowd evacuation scenario (simulated agents leaving)
      const totalAgents = 24;
      const escapeRate = 0.4; // agents per second
      const escaped = Math.min(totalAgents, Math.floor(time * escapeRate));
      const active = totalAgents - escaped;
      
      setMetrics({
        'Total Agents': totalAgents,
        'Active Agents Inside': active,
        'Evacuated Agents': escaped,
        'Est. Escape Clearance': escaped === totalAgents ? '100%' : `${Math.round(escaped / totalAgents * 100)}%`
      });

      // Change rooms visibility overrides or colors based on crowding
      if (active > 15) {
        setEntitySimState('living_room', { color: '#ef4444' }); // red crowding
      } else if (active > 5) {
        setEntitySimState('living_room', { color: '#f59e0b' }); // orange crowding
      } else {
        setEntitySimState('living_room', { color: '#10b981' }); // green crowding
      }

      // Safe evacuations events logs
        ...
        if (!events.some(e => e.id === eventId)) {
          addEvent({
            ...
            description: escaped === totalAgents 
              ? `All ${totalAgents} agents fully cleared. Structures cleared.`
              : `Evacuation progress: ${escaped} agents reached master exit assembly.`,
          });
        }
      }
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', height: '100%' }}>
      ...
      <div className="flex-col">
        <label className="text-muted" style={{ fontSize: '11px', fontFamily: 'var(--font-mono)' }}>ACTIVE SCENARIO</label>
        <select
        ...
        >
          ...
          {!domainSchema?.supportedSimulations.length && (
            <option value="">No simulations for this domain</option>
          )}
        </select>
      </div>
      ...
      <div className="sim-controls">
        <div style={{ display: 'flex', gap: '6px' }}>
          <button className="btn btn-small primary" onClick={handlePlayPause}>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '6px' }}>
          {Object.entries(metrics).map(([key, val]) => (
            <div key={key} className="sim-metric-card">
              <span style={{ fontSize: '10px', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>{key}</span>
              <span className="sim-metric-val">{val}</span>
            </div>
          ))}
          {Object.keys(metrics).length === 0 && (
            <div className="text-muted" style={{ gridColumn: 'span 2', padding: '6px', fontSize: '11px' }}>
              Press Play to gather telemetry.
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '120px' }}>
        ...
        >
          {events.map((evt) => (
            <div key={evt.id} style={{ display: 'flex', gap: '6px', borderBottom: '1px solid rgba(255, 255, 255, 0.02)', paddingBottom: '4px' }}>
              <span style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', flexShrink: 0 }}>
                [{evt.time.toFixed(1)}s]
              </span>
              <span style={{ color: getEventIconColor(evt.type), fontWeight: 'bold', flexShrink: 0 }}>
                �-�
              </span>
