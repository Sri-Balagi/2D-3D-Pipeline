...
const LoadingScanner: React.FC<{ progress: number }> = ({ progress }) => {
  const scanRef = useRef<THREE.Mesh>(null);
  
  useFrame(({ clock }) => {
    if (scanRef.current) {
      scanRef.current.position.y = Math.sin(clock.getElapsedTime() * 2.5) * 1.2;
    }
  });

  return (
    <group>
      {/* Outer bounding volume */}
      <mesh>
      ...
      {/* Corner indicators */}
      <gridHelper args={[2.5, 4, '#7C3AED', '#21153A']} position={[0, -1.25, 0]} />
      <gridHelper args={[2.5, 4, '#7C3AED', '#21153A']} position={[0, 1.25, 0]} />

      {/* Dynamic Laser Scanning Sheet */}
      <mesh ref={scanRef}>
        <boxGeometry args={[2.52, 0.03, 2.52]} />
        <meshBasicMaterial color="#DDD6FE" transparent opacity={0.5} />
      </mesh>
      ...
      <Html distanceFactor={6} center>
        <div 
          style={{ 
            ...
            border: '1px solid var(--vf-border-active)',
            borderRadius: '4px',
            padding: '8px 12px',
            display: 'flex',
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '20px' }}>
            <span style={{ color: 'var(--vf-gold-soft)', fontWeight: 'bold' }}>● SPATIAL_LOAD</span>
            <span>{progress > 0 ? `${progress}%` : 'INIT'}</span>
          </div>
          <div style={{ width: '100px', height: '3px', background: 'var(--vf-deep-violet)', borderRadius: '1.5px', overflow: 'hidden' }}>
            <div style={{ width: `${progress}%`, height: '100%', background: 'var(--vf-violet)' }} />
          </div>
          <span style={{ color: 'var(--vf-text-muted)', fontSize: '8px' }}>
            {progress < 30 ? 'MODEL INPUT...' : (progress < 70 ? 'SPATIAL ANALYSIS...' : 'WORLD INITIALIZATION...')}
          </span>
        </div>
      </Html>
    </group>
...
interface GLTFModelProps {
  ...
  visualizationMode: VisualizationMode | 'ghost-isolated';
...
export const GLTFModel: React.FC<GLTFModelProps> = ({
  ...
  if (loading) {
    return <LoadingScanner progress={progress} />;
  }
      ...
      <group position={[0, 0, 0]}>
        ...
        <Html distanceFactor={8} center>
          <div style={{
            ...
            <div style={{ fontWeight: 'bold', color: 'var(--accent-red)', marginBottom: '4px' }}>⚠ ASSET UNAVAILABLE</div>
